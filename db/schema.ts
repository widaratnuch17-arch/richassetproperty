...
export const propertyInquiries = sqliteTable(
  "property_inquiries",
  {
    ...
    commissionIncome: integer("commission_income"),
    dealExpenses: integer("deal_expenses").notNull().default(0),
  ...
  (table) => [
    ...
    index("idx_property_inquiries_closed_at").on(table.closedAt),
  ],
);
...
export const buyerRequests = sqliteTable(
  ...
  (table) => [
    index("idx_buyer_requests_status_created_at").on(table.status, table.createdAt),
    index("idx_buyer_requests_source_created_at").on(table.source, table.createdAt),
    index("idx_buyer_requests_next_follow_up").on(table.nextFollowUp),
  ],
);
